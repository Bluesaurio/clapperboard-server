...
router.post("/register", async (req,res,next) => {
  ...
  if (passwordRegex.test(password) === false) {
    ...
    if (username.length < 6) {
        res.status(400).json({errorMessage: "El nombre de usuario debe tener mínimo 6 carácteres"})
    }
    ...
    try {
        ...
        if (foundUser) {
            res.status(400).json({errorMessage: "El nombre de usuario introducido ya está en uso."}) 
        }

        if (foundEmail) {
            res.status(400).json({errorMessage: "El email introducido ya está en uso"})
        }
